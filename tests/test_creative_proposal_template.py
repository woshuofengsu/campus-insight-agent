...
def test_form_status_declaration_present():
    """五.1 必须明确写清"这是什么形态"：演示级原型（Demo），不是生产项目、无真实试点。

    为什么单独立一条：材料里最容易出现的失分不是数据好看不好看，而是**形态含糊**——
    读的人以为"已经上线在社区用了"。所以要求这句话必须直白写出来，且与"演示数据"口径一致。
    """
    sec = _section(_doc(), "### 1. 原型状态", "### 2.")
    for key in ("演示级原型", "不是已上线的生产项目"):
        assert key in sec, f"五.1 缺少形态声明关键词：{key}"
    assert ("无真实社区试点" in sec) or ("无真实用户数据" in sec), \
        "形态声明里必须点明「无真实试点/无真实用户数据」"
    assert "不是（我们不宣称）" in sec, "形态声明应有「是/不是」对照，明确我们**不**宣称什么"
    # 还要"开头就能看见"：不能只藏在第五章，评审翻第一页就该知道这是 Demo
    head = _doc()[:1200]
    assert "演示级原型" in head and "不是已上线的生产项目" in head, \
        "文件开头（前 1200 字）必须直接写明形态，避免评审误以为已上线"


def test_mobile_tradeoffs_documented():
    """移动端适配的让步必须逐条写清（用户明确要求：适配上做的让步要写出来）。

    覆盖十项里最容易被追问的七项：不做 App/无离线、大屏不适配、横屏不重排、
    热区放大导致密度下降、语音只算增强、动效削减、真机未验证。
    """
    sec = _section(_doc(), "### 1. 原型状态", "### 2.")
    assert "移动端适配" in sec and "让步" in sec, "五.1 缺少「移动端适配的让步」小节"
    for key in ("不做原生 App", "无 service worker", "治理大屏在手机上不做适配",
                "老年端横屏不做重排", "热区放大换来信息密度下降",
                "语音是增强项、不是承诺", "真机验证尚未完成"):
        assert key in sec, f"移动端让步清单缺条目：{key}"
    assert "不宣称离线" in sec, "必须写明「不宣称离线」这一口径（与口径门禁一致）"
