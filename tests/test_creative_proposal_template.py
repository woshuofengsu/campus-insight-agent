...
def test_form_status_declaration_present():
    """五.1 必须明确写清项目形态：演示级原型（Demo），不是生产项目、无真实试点。

    为什么单独立一条：提交件最容易出现的失分不是数据好不好看，而是**形态含糊**——
    读的人以为"已经上线在社区用了"。所以这句话必须直白写出来，且与"演示数据"口径一致。
    """
    ...
    assert "已实现（真实运行）" in sec and "尚未具备" in sec, \
        "形态声明应用「已实现 / 尚未具备」对照，明确哪些能力真的存在"
    # 开头就要能看见：不能只藏在第五章
    head = _doc()[:1200]
...
def test_mobile_tradeoffs_documented():
    """移动端适配的取舍必须逐条写清（用户明确要求：适配上做的让步要写出来）。

    覆盖十项里最容易被追问的七项：不做 App/无离线、大屏不适配、横屏不重排、
    热区放大导致密度下降、语音只算增强、动效削减、真机未验证。
    """
    sec = _section(_doc(), "### 1. 原型状态", "### 2.")
    assert "移动端适配" in sec and "取舍" in sec, "五.1 缺少「移动端适配的取舍」小节"
    for key in ("不做原生 App", "无 service worker", "治理大屏在手机上不做适配",
                "老年端横屏不做重排", "信息密度相应下降",
                "语音为增强功能", "真机验证尚未完成"):
        assert key in sec, f"移动端取舍清单缺条目：{key}"
    assert "不提供离线能力" in sec, "必须写明「不提供离线能力」这一口径"
