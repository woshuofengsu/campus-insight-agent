<script setup>
// 全局极端天气滚动提醒（居民端/网格员端所有页面顶部，可临时关闭 6 小时）
import { ref, onMounted, onUnmounted } from 'vue'
import { weather } from '../api'

const alerts = ref([])
const hidden = ref(localStorage.getItem('ci_banner_hidden') === '1')
const tick = ref(0)
let timer = null
...
function closeBanner() {
  hidden.value = true
  localStorage.setItem('ci_banner_hidden', '1')
}
</script>
...
<template>
  <div v-if="alerts.length && !hidden" style="background:#dc2626;color:#fff;padding:8px 12px;overflow:hidden;font-size:0.9rem;">
    <div style="display:flex;align-items:center;gap:10px;">
      ...
      <button @click="closeBanner" style="background:rgba(255,255,255,0.2);border:none;color:#fff;border-radius:99px;padding:2px 12px;cursor:pointer;font-size:0.8rem;">✕ 关闭</button>
    </div>
  </div>
</template>
