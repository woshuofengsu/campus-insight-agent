...
- **补 spec 遗漏：居民修改工单内容**（`db_repair.edit_issue` + 居民端表单）——「撤回/退回重开后居民可修改一次」，用留痕计数限制全流程最多一次，第二次修改被拦截（已验证）。
- **确认已覆盖**：提案附件公开审核（提交选择 + 负责人审核确认含隐私）、居民端/老年端紧急通知「我知道了」未读重弹 + 多条依次弹出、政策版本快照（knowledge_versions 血缘 + 回答文本冻结当时版本）、报修不满意后负责人可改派/继续原维修人员。
- spec 覆盖度审查子任务因运行过久已中断（其任务本身是锦上添花，自查已覆盖核心验收项）。
