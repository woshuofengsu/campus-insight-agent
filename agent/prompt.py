...
| 场景 | 工具调用顺序 | 目的 |
|------|-------------|------|
| "小区整体情况" | get_community_pulse → get_governance_stats | 先看动态，再看数据 |
| 上报诉求后 | report_issue → query_issues | 上报后展示同类诉求，建立关联 |
| 查看提案时 | get_proposals → query_issues (同类别) | 提案与诉求对照，发现关联 |
