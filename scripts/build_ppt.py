...
结构（17 页）：
  正文 15 页 封面 / 痛点 / 方案总览 / 架构与数据流 / 多智能体与双层防线 /
             居民端·报修 / 居民端·问与议 / 网格员端·工作台 / 网格员端·处理与关怀 /
             老人端·看得见按得动 / 老人端·不会打字也能办事 / 功能一览（分端）/ 数字墙 /
             位置与诉求 / 结尾
...
def build(tests, golden):
    ...
    text(s, 1.1, 4.4, 6.6, 0.7, ["步承泽 · 北京工商大学"], size=24, bold=True, color=WHITE)
    ...
    # ================= 正文 6：居民端（一）报修 =================
    s = prs.slides.add_slide(prs.slide_layouts[6])
    title(s, "居民端：报修，一句话就能说清")
    text(s, 0.95, 1.72, 11.4, 1.2,
         ["说一句「我家厨房水管漏水了」，它整理成报修单；",
          "要我自己点「确认提交」才提交——我不让它自己提交。"], size=22, spacing=1.25)
    shots = [
        ("18-居民端-一句话报修.png", "① 一句话，AI 理出问题/分类/紧急程度"),
        ("19-居民端-报修已生成.png", "② 我确认后，工单才生成"),
        ("10-提交报修表单.png", "③ 也可以手动填表提交"),
    ]
    gx = 0.95
    for name, cap in shots:
        pic(s, name, gx, 3.1, w=3.6)
        text(s, gx, 5.45, 3.6, 0.8, [cap], size=15, color=GREY)
        gx += 3.9
    text(s, 0.95, 6.35, 11.4, 0.5, ["识别到安全隐患，自动升级成紧急通知"],
    ...
    pic(s, "17-网格员端-工单处理.png", 0.95, 1.8, w=5.4)
    pic(s, "14-网格员端-老年关怀.png", 6.95, 1.8, w=5.4)
    text(s, 0.95, 5.35, 5.4, 0.7, ["工单处理：审核 / 派单 / 处理都在这"], size=16, color=GREY)
    ...
    text(s, 1.2, 6.5, 11, 0.6, ["这三个问题我回去改。"], size=20, color=GREY)
    page_no(s, 14)

    # ================= 正文 15：结尾 =================
    s = prs.slides.add_slide(prs.slide_layouts[6])
    text(s, 0.95, 1.85, 11.4, 1.3, ["谢谢各位"], size=54, bold=True, color=NAVY, align=PP_ALIGN.CENTER)
    box(s, 3.2, 3.55, 6.8, 1.85, LIGHT)
    text(s, 3.45, 3.8, 6.3, 1.4,
         ["社区先知 CommunityInsight", "基层治理 · 网格化多智能体平台", "居民端 / 网格员端 / 老人端"],
         size=19, align=PP_ALIGN.CENTER, spacing=1.35)
    text(s, 0.95, 5.75, 11.4, 0.5,
         ["步承泽 · 北京工商大学 · 一个人做完需求、架构、开发与测试"],
         size=18, color=INK, align=PP_ALIGN.CENTER)
    text(s, 0.95, 6.3, 11.4, 0.5,
         ["全部验证命令都在仓库里，欢迎现场复算"],
    ...
    # ================= 讲者备注（顺序与页面一一对应） =================
    notes = [
        "封面（开场 25 秒 + 自我介绍 7 秒；**PPT 先别翻**）：上周，我家楼下张奶奶摔了一跤。她想找人帮忙，手机却只会打给外地的儿子。等社区知道这件事，天已经黑了。——这一晚没有人做错什么，是这条路太长了。我想把它缩短成一句话，这就是社区先知。（这时才翻到封面）各位专家好，我是步承泽，北京工商大学大二学生。这个项目，从需求、架构到开发与测试都是我一个人做的。它服务三种人：居民、网格员、老人。今天七分钟，我讲三件事：怎么做、能干什么、我卡在哪。",
        ...
        "居民端（一）报修：说一句「我家厨房水管漏水了」，它只追问关键项，然后把报修单整理好（问题 / 分类 / 紧急程度），**要居民自己点「确认提交」才真的提交——我不让它自己提交**。也支持手动填表提交。识别到安全隐患（比如燃气味）会自动升级成紧急通知。",
        ...
        "网格员端（二）处理与关怀：工单管理能搜索、能导出、能批量派单、能批量关闭；**点开一行工单，审核 / 派单 / 处理都在这里做**，退回必须写意见，派单必须填维修人员与电话；每一步都有留痕，事后能复盘。老年关怀里，用药提醒和紧急联系人都要审核通过才生效；老人求助来了，要确认响应、再结束。",
        ...
        "它现在的位置：工程原型，没在真实街道试点，没有运营数据，商业模式在探索。今天不是来要投资，是来请教三个问题：多智能体的自动边界怎么划、第一个试点社区怎么谈、没有历史数据怎么冷启动。这三个问题我回去改。",
        "结尾（10 秒）：停一下，看着台下说「张奶奶那条路，现在是一句话」，再说「谢谢各位」。说完就停，不要补话。（屏幕上只有「谢谢各位」，这句话是**说出来的**。）",
        "附录：若对方想知道怎么在电脑上看实物，翻到这页。",
        "附录：四个最常被问的问题与诚实回答，独立阅读时直接看这页。",
    ]
    # 备注与页面必须一一对应。这里以前写过 `if i < len(prs.slides)`，
    # 结果备注条数比页数多一条时**整体错位一格**且无人察觉——所以改成硬校验、不许静默。
    if len(notes) != len(prs.slides):
        raise SystemExit(
            f"❌ 讲者备注 {len(notes)} 条 != 幻灯片 {len(prs.slides)} 页：会导致备注整体错位，请先对齐再生成。"
        )
...
def main() -> int:
    ...
    print(f"   共 {n_slides} 页 = 正文 {MAIN_PAGES} 页 + 附录 2 页；"
