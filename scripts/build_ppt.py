...
def build(tests, golden):
    ...
    # ================= 正文 15：结尾 =================
    s = prs.slides.add_slide(prs.slide_layouts[6])
    text(s, 0.95, 1.55, 11.4, 1.1, ["谢谢各位"], size=48, bold=True, color=NAVY, align=PP_ALIGN.CENTER)
    text(s, 0.95, 2.85, 11.4, 0.9, ["张奶奶那条路，现在是一句话。"],
         size=30, color=ORANGE, bold=True, align=PP_ALIGN.CENTER)
    box(s, 3.5, 4.15, 6.2, 1.45, LIGHT)
    text(s, 3.75, 4.32, 5.7, 1.15,
         ["社区先知 CommunityInsight", "基层治理 · 网格化多智能体平台", "居民端 / 网格员端 / 老人端"],
         size=17, align=PP_ALIGN.CENTER, spacing=1.3)
    text(s, 0.95, 5.95, 11.4, 0.5,
         ["步承泽 · 北京工商大学 · 一个人做完需求、架构、开发与测试 · 验证命令都在仓库里，欢迎现场复算"],
    ...
    notes = [
        ...
        "结尾（10 秒）：停一下，看着台下说「张奶奶那条路，现在是一句话」，再说「谢谢各位」。说完就停，不要补话。",
