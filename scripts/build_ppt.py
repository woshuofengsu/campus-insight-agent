...
def build(tests, golden):
    ...
    feats = [
        ...
        ("通知", "普通 / 紧急 / 定时 / 已读回执"),
        ("政策问答", "答案带出处；检索无结果就转人工"),
        ("健康咨询", "只给建议，紧急情况转人工（不做诊断）"),
        ("天气联动", "极端天气 → 健康防护建议 → 自动发通知"),
        ("网格员工作台", "AI 助手、待办、统计导出"),
        ("转人工", "带「处理包」：用户信息 + 执行链 + 待确认事项"),
    ]
    ...
    notes = [
        ...
        "老人端是单独重做的，不是把居民端放大：大字大按钮、免登录。不会打字也不怕，常用的话做成大按钮点一下就问（家里灯不亮了 / 医保怎么报销）。吃药提醒到点会响，就两个按钮：我吃了、10 分钟后再说；用药提醒要社区审核通过才开始播报。紧急求助长按 3 秒出确认框，10 秒不点自动取消——我怕老人放兜里误触，所以防了两道。",
        "五步走一遍，都是真实界面。① 居民报修：一句话提交，不用选分类。② 政策问答：答案下面有依据，写明哪份政策、适用哪个地区；答不出依据就不硬答，转人工。③ 天气联动：点开执行链，天气守护、健康顾问、通知管理三个角色都在，谁先动谁接话看得见。④ 老人端：长按 3 秒出确认框（我不能真发，点到取消）。⑤ 网格员端：刚才那条报修已进工作台，待办、AI 助手、统计导出都在这一页。同一件事，居民、老人、网格员三边是通的。",
        "再看三处细节（五步之外的补充）：政策问答的答案下面有知识库依据和适用地区；工单详情里能看到谁在什么时候接的、什么时候处理的，全程留痕；居民端报修列表每条走到哪一步、状态和颜色都写清楚。",
