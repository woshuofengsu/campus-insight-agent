...
### 2026-09-24 · B3：老年端 P4 健康记录（血压/血糖）

- 迁移 **v47** 新建 `elderly_vitals` 流水表（不用 `health_info` JSON：那是整块覆盖写，并发录入会丢记录），
  并把旧档案里的历史血压幂等回填进新表。
- 新增 `data/_vitals_logic.py`（纯逻辑：阈值分级 + **固定文案唯一出口**）与 `data/db_vitals.py`
  （录入校验含荒谬值拦截、列表、趋势、异常提醒）。
- **只提醒不诊断**：文案固定"建议联系社区医生或家属复核，必要时就医"，测试用黑名单正则拦住诊断句式；
  异常只在 alert 级发通知，每人每天每种指标最多一条（按 UTC 日期去重）。
- 接口：老年端录入/列表/摘要 + 负责人端 `manage/vitals`、`manage/elders`（居民越权返回 1003）。
- 前端：老年端新页「我的健康」（大字录入 + 最近记录 + 听一遍），首页九宫格换成"🩺 我的健康"；
  网格员端新增「🩺 健康记录」页签。
- 审计清单补齐（`elderly-health`、`grid-elderly-care`、health 暗色）后，门禁抓出两处新页问题并已修：
  字号未达老年端 20px、**内联写死十六进制色导致暗色对比度 2.91:1**（改用 `--ink-*` 成对令牌）。
- 门禁：全量用例全绿 · 移动端审计（22 页）全过 · 全站 UI 审计 0 处 HIGH。

### 2026-09-24 · B4：老年端无障碍/降级硬化（语音不可用也能办事）

- `useSpeech.js` 新增 `speechCapability()`（同步探测 ASR/TTS/安全上下文）与 `reasonText()`（降级文案唯一出口）；
  `speak()` 返回值语义明确，承载信息的调用点必须处理 false。
- 六个用语音的页面统一加降级提示条（`data-speech-fallback`）；报告页/问答页不再把"不支持"误报成"没听清"。
- **承载信息的播报改为"点一下听"**（iOS Safari 的 TTS 必须由用户手势触发，原来挂载即播报会静默失败）：
  首页问候/关怀、紧急通知弹窗、通知页紧急通知；语音不可用时隐藏话筒按钮并聚焦打字框。
- `scripts/mobile_audit.py` 新增第 ⑧ 项检查：删掉全部语音 API 后重载，断言提示条出现且仍有可操作控件
  ——这是"降级路径真的做了"的机器证据（6 个语音页面全过）。
...
### 2026-09-24 · B6：多租户按-id 越权收口 + 写入侧补漏（B5 的复查）

B5 做完"列表隔离"后**自测发现还有口子**：列表走了 SQL 过滤，但**详情/操作接口是按 id 直取单行的**，
那条路径没有 WHERE 可加、原实现只校验角色（"是网格员就放行"）。实测复现：朝阳试点社区网格员
`GET /api/web/issues/352` 返回 **HTTP 200 + 海淀工单诉求全文**（`scripts/probe_tenant_isolation.py` 有前后对比）。

- **按-id 授权闸**：新增 `utils.tenant.row_in_tenant`（表名白名单 + 全分支 fail-closed）与
  `api_routes.deps._same_tenant(request, 表, 行id)`，在 **21 处**详情/操作路由上收口：
  工单（详情/管理动作）、提案（详情/投票/议论列表/发表议论/管理动作）、健康咨询（详情/回复）、
  通知（详情/管理动作）、政策提问（回复/删除/转人工）、老年端（用药修改与审核、联系人删除与审核、
  SOS 处置）、人工处理包（列表/关闭）、链路追踪。
  语义：行的租户 ≠ 请求者社区、行为空、行无租户、表名不在白名单 → **一律拒绝**（查不动也不放行）。
- **写入侧补漏 6 处**（B5 只覆盖了 9 处，漏的后果比"看不见别人的"更糟——**自己人也看不见**）：
  `agent_handoffs`（转人工处理包 → 列表已按租户过滤 → 所有网格员都看不到待办）、
  `policy_questions`（居民转人工提问 → 网格端待回复空列表）、`care_event_log`（关怀量化恒为 0）、
  `weather_check_tasks`（极端天气巡查任务列表为空，按默认社区归档并注明边界）、
  `community_issues`（政务上报入口 / 系统感知生成两处 → 工单在网格端列表消失）、
  老年端历史数据迁移两处。新增 `stamp_tenant_value`（无归属人行的盖章口，不写默认值）。
- **运行时按租户过滤补漏**：`list_handoffs`、`get_trace_chain`（agent_logs 部分）此前无租户参数。
- **两道防锈闸**（新加路由/插入点漏了会在测试期红，而不是演示当天）：
  `tests/test_tenant_idor_sweep.py`（41 例：判定契约 + 16 个按-id 接口跨租户全拒/同租户放行 +
  AST 扫描"按-id 路由必须带闸门"）、`tests/test_tenant_write_side.py`（16 例：AST 扫描
  "租户表插入必须盖章"+ 扫描器自检，防假绿闸门）。
- 验证：全量 **754 项可运行（753 passed / 1 skipped）**；`ruff` 0；真服务实测跨租户由 200 变 400。

### 2026-09-24 · B7：多租户配置隔离（settings 按社区分键）+ 提交件数字改准

- **配置隔离**：`settings` 表此前所有键都是全局一份（两社区共用天气联动阈值/政策匹配阈值）。
  新增 `data/db_settings.py`：`键@社区` = 社区专属、裸键 = 全局默认，读取顺序
  **社区专属 → 全局 → 代码默认**，**写入只写社区键**；社区名含 `@`（键解析歧义）明确报错。
- **判定点接线**（否则就是"配了不生效"）：政策阈值按**提问人所属社区**取（接进 `ask_question` 判定点）；
  天气联动阈值接进 `weather_event_to_link_keys`；联动关闭状态与"今日已触发"去重**都按社区各记各的**
  （留痕带 `[社区]` 标签；`activity_log` 无 tenant 列，不改表结构）；
  **定时任务按社区逐个判定**（`all_tenants()` 枚举，无社区时回落一次全局调用）——
  原来只调一次不带社区，等于"按社区配阈值"永远不生效。
- **去掉跨租户串味的根源**：`_match_threshold` / `_LINKAGE_THRESHOLDS` 不再被 setter 改写
  （此前朝阳改配置会连带改掉海淀）。
- **顺手修两个"做了但没接上主路径"**：① 联动的 `permanent`（永久关闭）**没有任何入口**，
  导致 `reopen` 永远没有可开启的对象 → 请求体补 `permanent` 开关并接上；
  ② 无调用方的历史接口 `get_elderly_linkage_reminders` 补 `tenant` 参数与风险注释。
- **提交件数字改准**：7 份「当前状态文档」仍是 v46/130 路由/50 表/46 迁移，全部改准为
  **v48 / 135 / 51 / 47**；并把旧值加进 `check_claims` 的过时表述黑名单防回流
  （CHANGELOG 与 HANDOFF 作为历史记录豁免）；顺手把 `test_claims_consistency.py` 里
